...
export default function TestClient() {
    ...
    const [visualLog, setVisualLog] = useState(['log start'])
    ...
    async function constructAndRegisterPeer(){
    ...
    async function connectToPeer(currentPeer, remotePeerId){
        ...
        remoteConnection.on('open', () => {
            ...
            remoteConnection.send(`CONNEGGTED - notification from ${peerId}`)
        })
    ...
    return (
        <>
            ...
            <button onClick={async (event)=>{setPeer(await constructAndRegisterPeer())}} >register with peerserver</button>
            {peer && <>
                <input type="text" onChange={(event)=>{setPeerIdInput(event.target.value)}} value={peerIdInput}/><button onClick={async (event)=>{connectToPeer(peer, peerIdInput)}}> connect to peer</button>
            </>}
            <div>my id: </div>
            <div>{peerId}</div>
            {peer && <>
                <input type="text" onChange={(event)=>{setChatInput(event.target.value)}} value={chatInput}/><button onClick={async (event)=>{peer.send(chatInput)}}>send chat</button>
            </>}
            <hr />
            <div className="visual-log">{visualLog.map((item, index) => <div key={index}>{item}</div>)}</div>
